...
import './forum.css';
import {Row, Col, Card, Tabs, Avatar,Button,Modal,Form,Input,Select} from 'antd';
...
import {getForums, getForumsPopular, getNews, postForum} from "../../server/config/Servises";
...
class Forum extends React.Component{
    ...
    getForumsList=()=>{
        const {tapKey,size}=this.state;
        if (tapKey=='1' || tapKey=='3'){
            getForums(0,size).then((res)=>{
                if(res){
                    this.setState({
    ...
    render() {
        const {batafsil,isModalVisible,forums,size,news,disabled}=this.state;
        ...
        const onFinishForum=(values)=>{
            ...
            document.getElementById("forum_form").reset();
        }
        const ShowMoreForum=()=>{
            const {tapKey,size}=this.state;
            this.setState({
        ...
        return(
            <React.Fragment>
                <TransverseLoading size={'30px'}/>
                <div className="main_forum">
                    <div className="container">
                        <Modal title="Question" className={'commentModal'} visible={isModalVisible} onOk={onFinishForum} onCancel={handleOk}>
                            <Form
                                style={{
                                    width:'100%',
                                    position:'absolute'
                            ...
                            >
                                ...
                                <Form.Item
                                    ...
                                    rules={[{ required: true, message: 'Please input your question!' }]}
                                >
                                    <TextArea className={'title_forum'}/>
                                </Form.Item>
                        ...
                        <h3><Link to={'/home'}>{uzLang?"Bosh sahifa":enLang?"Home":"Главная"}</Link>{'>'}{uzLang?"Forum":enLang?"Forum":"Форум"}</h3>
                        <Row>
                                    ...
                                    <Tabs defaultActiveKey="1" onChange={this.onchangeTab}>
                                        ...
                                        <TabPane tab={<h3>  {uzLang?"Barchasi":enLang?"All":"Все"}</h3>} key="1">
                                            <div className="select_konkurs">
                                                <Row>
                                                    <Col span={2}>
                                                        <Select defaultValue={'Barchasi'} onChange={Select_filtr}>
                                          ...
                                          <All forums={forums}/>
                                            <Button onClick={ShowMoreForum} className={'video_all'}>{uzLang?"Ko'proq ko'rsatish":enLang?"Show more":"Показать еще"}</Button>
