import React from 'react';
import './AddQuestion.css';
import {Row, Col, Card, Tabs, Avatar,Button,Input} from 'antd';
import {ArrowDownOutlined,ArrowUpOutlined ,EyeOutlined,MessageOutlined,ArrowRightOutlined,LikeOutlined,DislikeOutlined} from '@ant-design/icons'
import {connect} from "react-redux";
import {uzLanguage} from "../../Redux/Actions/Uzlanguage";
import {ruLanguage} from "../../Redux/Actions/RuLanguage";
import {enLanguage} from "../../Redux/Actions/EnLanguage";
import {
    getConsultationById,
    getConsultationRepliesByReplyId, getConsultationRepliesByReplyIdAll,
    getForumById,
    getForumRepliesByReplyId, getForums, postForum
} from "../../server/config/Servises";
import {Link} from "react-router-dom";
const {TabPane}=Tabs;
const {Meta}=Card;
const {TextArea }=Input;

class Addquestion extends React.Component{
    state={
        batafsil:false,
        forum:{},
        commentsForum:[],
        size:2,
        CommentValue:'',
        twoForum:[],
    };
    componentDidMount() {
        getForumById(window.location.pathname.split('/')[2]).then((res)=>{
            if(res){
                console.log(res)
                this.setState({
                    forum:res.data,
                })
            }
        });

        getForumRepliesByReplyId(window.location.pathname.split('/')[2],0,this.state.size).then((res)=>{
            if(res){
                this.setState({
                    commentsForum:res.data.content,
                })
            }
        });
        getForums(0,2).then((res)=>{
            if(res){
                this.setState({
                    twoForum:res.data.content,
                })
            }
        })

    }
    render() {
        const {batafsil,forum,commentsForum,size,CommentValue}=this.state;
        const {uzLang,enLang,current_user}=this.props;
        const callback=(key)=>{
            console.log(key)
        }
        const Batafsil=()=>{
            this.setState({
                batafsil:!batafsil
            })
        }
        const GetCommentsAll=()=>{
            document.querySelector('.see_all_comments').classList.add('close')
            document.querySelector('.close_all_comments').classList.remove('close')
            getConsultationRepliesByReplyIdAll(window.location.pathname.split('/')[2]).then((res)=>{
                if(res){
                    this.setState({
                        commentsForum:res.data.content,
                    })
                }
            })
        }
        const changeInput=(e)=>{
            const {name,value}=e.target;
            this.setState({
                [name]:value,
            })
        }
        const CloseCommentsAll=()=>{
            document.querySelector('.see_all_comments').classList.remove('close')
            document.querySelector('.close_all_comments').classList.add('close')
            getForumRepliesByReplyId(window.location.pathname.split('/')[2],0,this.state.size).then((res)=>{
                if(res){
                    this.setState({
                        commentsForum:res.data.content,
                    })
                }
            })
        }
        const PostForumComment=()=>{
            const data={
                "title":'',
                "replyId":window.location.pathname.split('/')[2],
                "content":CommentValue,
                "tags":[],
            }

            if(document.querySelector('.content_forum').value!==''){
                postForum(data).then((res)=>{
                    if(res){

                        getForumRepliesByReplyId(window.location.pathname.split('/')[2],0,this.state.size).then((res)=>{
                            if(res){
                                this.setState({
                                    commentsForum:res.data.content,
                                })
                            }
                        })
                        this.setState({
                            CommentValue:'',
                        })

                    }
                })


            }




        }
        const ShowMoreComment=()=>{
            this.setState({
                size:size+2,
            })
            getForumRepliesByReplyId(window.location.pathname.split('/')[2],0,this.state.size).then((res)=>{
                if(res){
                    this.setState({
                        commentsForum:res.data.content,
                    })
                }
            })
        }

        return(
            <React.Fragment>
                <div className="main_forum">
                    <div className="container">
                        <h3>{uzLang?"Bosh sahifa":enLang?"Home":"Главная"}{'>'}{uzLang?"Forum":enLang?"Forum":"Форум"}</h3>
                        <Row>
                            <Col md={24} lg={18} sm={24}>
                                <Card>

                                    <div>
                                        <Row>
                                            <Col span={24}>
                                                <Card style={{ width: '90%',marginLeft:'5%', marginTop: 16,marginBottom:'20px' }} >
                                                    <Meta
                                                        width={'100%'}
                                                        avatar={
                                                            <Avatar src="data:image/jpeg;base64," style={{width:'70px',height:'70px'}} />
                                                        }
                                                        title={forum.userFullname}
                                                    />
                                                    <br/>
                                                    <h3>
                                                        {forum.title}
                                                    </h3>
                                                    <h4 style={{color:'#61666D'}}>
                                                        <h4>{forum.content}</h4><div>
                                                            </div>

                                                    </h4>
                                                    <br/>
                                                    {
                                                        forum.tags?forum.tags.map(item=>{
                                                            return <a href="#">#{item} </a>
                                                        }):''
                                                    }

                                                    <br/><br/>
                                                    <Row>
                                                        <Col span={18}>
                                                            <Row>
                                                                <Col span={6}>
                                                                    <div style={{color:'#50C878',fontSize:'18px',cursor:'pointer'}}><ArrowUpOutlined />{forum.replyCount}</div>
                                                                </Col>

                                                                <Col span={6}>
                                                                    <div style={{color:'#61666D',fontSize:'18px',cursor:'pointer'}}><ArrowDownOutlined />{forum.replyCount}</div>
                                                                </Col>

                                                            </Row>
                                                        </Col>
                                                        <Col span={6}>
                                                            <Row>
                                                                <Col span={12}>
                                                                    <div style={{color:'#61666D',fontSize:'18px',cursor:'pointer'}}><EyeOutlined />{forum.viewCount}</div>
                                                                </Col>

                                                                <Col span={12}>
                                                                    <div style={{color:'#61666D',fontSize:'18px',cursor:'pointer'}}><MessageOutlined />{forum.replyCount}</div>
                                                                </Col>

                                                            </Row>
                                                        </Col>
                                                    </Row>
                                                </Card>

                                            </Col>

                                        </Row>
                                    </div>
                                    <Card>
                                        <Card style={{ width: '90%',marginLeft:'5%', marginTop: 25,marginBottom:'20px' }} >
                                            <Meta
                                                width={'100%'}
                                                avatar={
                                                    <Avatar src="data:image/jpeg;base64," style={{width:'70px',height:'70px'}} />
                                                }
                                                title={current_user?current_user.firstname+' '+current_user.lastname:''}

                                            />
                                            <TextArea name={"CommentValue"} className={'content_forum'} onChange={(e)=>changeInput(e)} value={CommentValue}  rows={4} style={{height:'50px',borderRadius:'10px',marginTop:'20px'}} placeholder={uzLang?"Javobingizni kiriting":enLang?"Enter your answer":"Введите свой ответ"} />
                                            <Button onClick={PostForumComment} className={'dobavit_btn '}>{uzLang?"Qo'shish":enLang?"Add to":"Добавить"}</Button>



                                        </Card>
                                        {
                                            commentsForum?commentsForum.map((item)=>{
                                                return   <Card style={{ width: '90%',marginLeft:'5%', marginTop: 16,marginBottom:'20px' }} >
                                                    <Meta
                                                        width={'100%'}
                                                        avatar={
                                                            <Avatar src="data:image/jpeg;base64," style={{width:'70px',height:'70px'}} />
                                                        }
                                                        title={item.userFullname}

                                                    />
                                                    <br/>
                                                    <h3>
                                                        {item.title}
                                                    </h3>
                                                    <h4 style={{color:'#61666D'}}>
                                                        <h4>{item.content}</h4><div>

                                                    </div>

                                                    </h4>

                                                    <Row>
                                                        <Col span={18}>
                                                            <Row>
                                                                <Col span={6} sm={12} xs={24} lg={6} >
                                                                    <div style={{color:'#61666D',fontSize:'18px',cursor:'pointer'}}><LikeOutlined />{uzLang?"Yoqdi":enLang?"Like":"Нравится"}</div>
                                                                </Col>



                                                            </Row>
                                                        </Col>

                                                    </Row>
                                                </Card>
                                            }):''
                                        }


                                        <Button onClick={ShowMoreComment} className={'video_all'}>{uzLang?"Ko'proq ko'rsatish":enLang?"Show more":"Показать еще"}</Button>


                                    </Card>
                                </Card>
                            </Col>
                            <Col md={12} lg={6} sm={24}>
                                <Card title={
                                    <div>
                                        <table>
                                            <tr>
                                                <td><h3> {uzLang?"Bildirishnoma":enLang?"Notification":"Уведомление"}</h3></td>
                                                <td><button className={'messages_forum'}>2</button></td>
                                                <td><ArrowRightOutlined style={{marginLeft:'30px',cursor:'pointer'}} /></td>
                                            </tr>
                                        </table>


                                    </div>
                                } style={{ width: '100%', marginLeft: 10,marginBottom:'100px' }} >
                                    {
                                        this.state.twoForum?this.state.twoForum.map(item=>{
                                            return <Meta
                                                width={'100%'}
                                                avatar={
                                                    <Avatar src="data:image/jpeg;base64," style={{width:'40px',height:'40px'}} />
                                                }
                                                title={<div>
                                                    <p style={{fontSize:'14px',color:'black',marginTop:'0px',display:'inline-block'}}>{item.userFullname} </p><p style={{color:'#61666D',fontSize:'12px'}}>{item.title}</p>
                                                </div>}
                                            />
                                        }):''
                                    }
                                    <br/>
                                    <table>
                                        <tr>
                                            <td><h3> {uzLang?"Yangiliklar":enLang?"News":"Новости"}</h3></td>
                                            <td><button className={'messages_forum'}>4</button></td>
                                            <td><ArrowRightOutlined style={{marginLeft:'30px',cursor:'pointer'}} /></td>
                                        </tr>
                                    </table>
                                    <p style={{color:'#61666D'}}>Выдали новое постановление про форму в школах. Теперь...</p>
                                    <p style={{color:'#61666D'}}>Выдали новое постановление про форму в школах. Теперь...</p>
                                    <p style={{color:'#61666D'}}>Выдали новое постановление про форму в школах. Теперь...</p>
                                    <br/>
                                    <table>
                                        <tr>
                                            <td><h3>  {uzLang?"Ommabop teglar":enLang?"Popular tags":"Популярные теги"}</h3></td>

                                        </tr>
                                    </table>
                                    <Row>
                                        <Col span={12}> <Button style={{borderRadius:'15px',marginBottom:'10px'}}>#история</Button></Col>
                                        <Col span={12}> <Button style={{borderRadius:'15px'}}>#история</Button></Col>
                                        <Col span={12}> <Button style={{borderRadius:'15px',marginBottom:'10px'}}>#история</Button></Col>
                                        <Col span={12}> <Button style={{borderRadius:'15px'}}>#история</Button></Col>
                                        <Col span={12}> <Button style={{borderRadius:'15px'}}>#история</Button></Col>
                                    </Row>
                                </Card>
                            </Col>
                        </Row>
                    </div>
                </div>
            </React.Fragment>
        )
    }

}
const mapStateToProps = (state) => {

    return {
        uzLang: state.changeLang.uzLang,
        enLang: state.changeLang.enLang,
    };
};
export default connect(mapStateToProps,{uzLanguage,ruLanguage,enLanguage})(
    Addquestion
)